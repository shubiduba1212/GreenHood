...
<?php
  //http://localhost/portfolio/shop_cate.php?category=bag&sort=num
  $category = $_GET["category"];
  $sort = $_GET["sort"];
  if($category=='acc'){
    $cate_tit = "Accessories";
  }elseif($category=='bag'){
    $cate_tit = "Bag";
  }elseif($category=='clothes'){
    $cate_tit = "Clothing";
  }elseif($category=='home'){
    $cate_tit = "Home "."&"." Living";
  }elseif($category=='etc'){
    $cate_tit = "etc";
  }
...
?>                     
            ...
            <ul class="shop_sort_type">             
              <li><a href="./shop_cate.php?category=<?=$category?>&sort=num">신상품순</a></li>
              <li><a href="./shop_cate.php?category=<?=$category?>&sort=hit">인기순</a></li>
              <li><a href="./shop_cate.php?category=<?=$category?>&sort=high_price">높은 가격순</a></li>
              <li><a href="./shop_cate.php?category=<?=$category?>&sort=low_price">낮은 가격순</a></li>
            </ul>
          </div>
          <ul class="shop_list">
<?php
  ...
  if($sort == "num"||$sort == "hit"){
    $sql = "select * from shopping where category='$category' order by $sort desc";
  }elseif($sort == "high_price"){
    $sql = "select * from shopping where category='$category' order by price desc";  
  }elseif($sort == "low_price"){
    $sql = "select * from shopping where category='$category' order by price";
  } 
  $result = mysqli_query($con, $sql);
  $total_record = mysqli_num_rows($result);
...
<?php
		...
		$new_page = $page - 1;
		echo "<li><a href='shop_list.php?category=<?=$category?>&sort=$sort&page=$new_page'>◀ 이전</a></li>";
 }else{  
		echo "<li>&nbsp;</li>";
	}
	...
	for($i=1; $i<=$total_page; $i++){
		if($page == $i){
			echo "<li><a href='shop_list.php?category=<?=$category?>&sort=$sort&page=$i' class='cur_page'> $i </a></li>";
		}else{
			echo "<li><a href='shop_list.php?category=<?=$category?>&sort=$sort&page=$i'> $i </a></li>";
		}
	}

	if($total_page >= 2 && $page != $total_page){
		$new_page = $page + 1;
		echo "<li><a href='shop_list.php?category=<?=$category?>&sort=$sort&page=$new_page'>다음 ▶</a></li>";
	}else{
		echo "<li>&nbsp;</li>";
	}
?>       </ul>   
